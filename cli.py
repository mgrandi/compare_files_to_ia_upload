...
from compare_files_to_ia_upload import app
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="compares local files with the files uploaded to internet archive (via the `_files.xml` file",
        epilog="Copyright 2020-08-31 - Mark Grandi")
    ...
    parser.add_argument('--files-xml-file',
        dest="files_xml_file",
        required=True,
        type=isFileType,
        help="the `_files.xml` file that describes the metadata about the IA collection")
    parser.add_argument("--local-files-folder",
        dest="local_files_folder",
        required=True,
        type=isFolderType,
        help="the folder containing the local copy of the files")
